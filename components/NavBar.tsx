...
export default function NavBar() {
  ...
  const darkMode = () => {
    ...
    htmlElement.dataset.theme =
      currentTheme === "winter" ? "dracula" : "winter";
  };
  const links = [
    { label: "Home", href: "/" },
    ...
    <nav className="navbar flex justify-between mb-2 bg-base-100/50">
      ...
      <div className="navbar-center hidden lg:flex">
        <ul>
          {links.map((link, i) => (
            <li
              key={i}
              className={`menu menu-horizontal border-base-content ${
                link.href === currentPath ? "border-b-2" : "border-b-0"
              } transition-all`}
            >
      ...
      <div className="navbar-end flex">
        <label className="swap swap-rotate btn-ghost btn btn-circle">
          ...
          {/* sun icon */}
          <svg
            className="swap-on fill-current w-8 aspect-square"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
          >
          ...
          {/* moon icon */}
          <svg
            className="swap-off fill-current w-8 aspect-square"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
          >
